...
public class CryptographyUtilsTests {
    ...
    public void testRandomString() {
        ...
        assertTrue(result.matches("^[ABC]+$"), "Random string should only contain characters from provided set");

        chars = "12345";
        result = CryptographyUtils.randomString(5, chars);
        assertEquals(5, result.length(), "Random string should have specified length");
        assertTrue(result.matches("^[1-5]+$"), "Random string should only contain characters from provided set");
    }
    ...
    public void testRandomBase64() {
        ...
        assertTrue(Pattern.matches(BASE64_PATTERN, base64), "Base64 string should contain valid Base64 characters");
